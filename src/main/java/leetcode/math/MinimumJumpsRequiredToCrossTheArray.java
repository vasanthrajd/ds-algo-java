 ...
 */
public class MinimumJumpsRequiredToCrossTheArray {
    public static void main(String[] args) {
        System.out.println("Minimum Jump Required to Cross the Array " + minimumJumps(new int[] {1, 3, 5, 8, -25, 2, 6, 7, 6, 8, 9}));
    }
    ...
    private static int minimumJumps(int[] arr) {
        ...
        for (int i=0; i<arr.length ; i++) {
            ...
            if (i == currentValue) {
                System.out.println("Current Value "+ currentValue + " nextJump " + nextJump);
                jumpCount++;
                currentValue = nextJump;

                // If the end of the array is within reach with the current jump
                if (currentValue >= arr.length - 1) {
                    return jumpCount;
                }
            }
